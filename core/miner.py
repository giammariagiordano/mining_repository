...
def normalize_path(path: str) -> str:
    """
    Normalizza i percorsi sostituendo sequenze miste di slash e backslash
    con un singolo slash (/).
    ...
    Se vuoi ottenere backslash finali (es. stile Windows), puoi cambiare
    l'ultima riga in: path = path.replace("/", "\\")
    """
    if not path:
        return path

    # 1. Porta tutto a slash forward
    path = path.replace("\\", "/")

    # 2. Comprimi sequenze multiple di slash in uno solo
    path = re.sub(r"/+", "/", path)
...
class RepositoryMiner:
    ...
    def mine(
        ...
        repo_stars: int,
        repo_forks: int,
        already_processed_shas: Set[str],
    ) -> pd.DataFrame:
        if self.config.analysis_mode == "commits":
            return self._mine_commits(
                repo, project_name, repo_path, repo_stars, repo_forks, already_processed_shas
            )
        elif self.config.analysis_mode == "releases":
            return self._mine_releases(
                repo, project_name, repo_path, repo_stars, repo_forks, already_processed_shas
            )
        elif self.config.analysis_mode == "version":
            return self._mine_single_version(
                repo, project_name, repo_path, repo_stars, repo_forks, already_processed_shas
            )
        else:
    ...
    def _mine_commits(
        ...
        try:
            for idx, commit in enumerate(commits, start=1):
                ...
                # Normalizza i path dei file
                modified_files = [normalize_path(f) for f in modified_files]
                ...
                print(
                    f"  [{project_name}] ({idx}/{len(commits)}) commit {sha[:7]} "
                    f"(new, LOC={repo_loc}, fix={is_fix})"
                )
                ...
                metrics = CommitMetrics(
                    ...
                    repo_path=sanitize_for_csv(normalize_path(repo_path), keep_chars="/\\:._-"),
                ...
                metrics.extra_metrics.update(
                    self.dpy_analyzer.run(repo.working_tree_dir, self.config.dpy_binary)
                )
                metrics.extra_metrics.update(
                    self.bandit_analyzer.run(repo.working_tree_dir, self.config.bandit_binary)
                )
                metrics.extra_metrics.update(
                    self.vulture_analyzer.run(repo.working_tree_dir, self.config.vulture_binary)
                )
                metrics.extra_metrics.update(
                    self.smell_ai_analyzer.run(repo.working_tree_dir, self.config.smell_ai_path)
                )
                ...
                if is_fix:
                    ...
                    for tag in fix_tags:
                        if tag.startswith("issue:"):
                            ...
                            comments = get_issue_comments(
                                project_name, issue_num, self.config.github_token
                            )
                            if comments:
                    ...
                    metrics.issue_comments = (
                        json.dumps(all_comments, ensure_ascii=False) if all_comments else ""
                    )
        ...
        if rows and self.openai_classifier.enabled:
            ...
            for author, author_commits in commits_by_author.items():
                ...
                df = pd.DataFrame(rows)  # o il punto in cui hai già il df pronto

# --- NORMALIZZAZIONE Smell AI files ---
                if "mlsmell_files" in df.columns:
                    df["mlsmell_files"] = df["mlsmell_files"].fillna(0)
                    df.loc[df["mlsmell_files"] == "", "mlsmell_files"] = 0
                # --------------------------------------
